#![Logo](https://github.com/xiaopansky/LinearLineWrapLayout/raw/master/res/drawable-mdpi/ic_launcher.png) LinearLineWrapLayout
...
>* 用法简单，支持在布局中直接包含View或者在代码中直接调用LinearLineWrapLayout.addTab()方法添加

## Sample Application
[Get it on Google Play](http://play.google.com/store/apps/details?id=me.xiaopan.android.LinearLineWrapLayout)

[Download it on Github](http://github.com/xiaopansky/LinearLineWrapLayout/raw/master/releases/LinearLineWrapLayout-1.1.0.apk)

##Usage Guide
####1.添加[LinearLineWrapLayout.java](http://github.com/xiaopansky/LinearLineWrapLayout/raw/master/src/me/xiaopan/android/LinearLineWrapLayout/LinearLineWrapLayout.java)到你的项目中

####2.添加[attrs.xml](http://github.com/xiaopansky/LinearLineWrapLayout/raw/master/res/values/attrs.xml)到你的项目的res/values目录下
如果你的项目中已经存在attrs.xml文件，那么你可以拷贝属性定义部分代码到你的attrs.xml文件中，代码如下：
```xml
<declare-styleable name="LinearLineWrapLayout">
    <attr name="slidingBlock" format="reference|color"/>
    <attr name="allowWidthFull" format="boolean"/>
</declare-styleable>
```
属性释义：
>* ``slidingBlock``：指定滑块图片，对应的方法是LinearLineWrapLayout.setSlidingBlockDrawable(Drawable)
>* ``allowWidthFull``：当所有Tab无法充满LinearLineWrapLayout时是否自动调整所有Tab的宽度来达到充满的目的，对应的方式是LinearLineWrapLayout.setAllowWidthFull(boolean)

####3.在布局中引用LinearLineWrapLayout
```xml
...
![smaple](https://github.com/xiaopansky/LinearLineWrapLayout/raw/master/docs/sample2.png)

你还可以在代码中通过LinearLineWrapLayout.addTab()方法添加Tab

####4.设置ViewPager
```java
ViewPager moreViewPager = (ViewPager) findViewById(R.id.viewPager);
moreViewPager.setAdapter(...);
LinearLineWrapLayout.setViewPager(moreViewPager);
```
注意：
...
<LinearLayout
    ...
    xmlns:app="http://schemas.android.com/apk/res/me.xiaopan.android.LinearLineWrapLayout"
    android:layout_width="match_parent"
